...
function BasicInfoTab(props) {
  ...
  useEffect(() => {
    ...
    const getBrand = async (a) => {

      const response = await fetch('https://www.laabamone.com/LingaChemicals/api.php?eventtype=brand&viewtype=listview&id=' + a);
      const json = await response.json();
      console.log('company', json);
      setname(json[0].name);
      setcode(json[0].Brand_Code);
  ...
  const SaveCompany = (event) => {
    ...
    console.log('https://www.laabamone.com/LingaChemicals/api.php?eventtype=brand'
      + str + '&name=' + name +
      '&code=' + code +
      '&userid=' + loginid + '&status=' + status)
    fetch('https://www.laabamone.com/LingaChemicals/api.php?eventtype=brand'
      + str + '&name=' + name +
      '&code=' + code +
      '&userid=' + loginid + '&status=' + status)
